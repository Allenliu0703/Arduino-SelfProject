    ...
    */
    ...
    if (modecontrol  == 1){  
      ...
      detachInterrupt(5);
          ...
          x = map(data[0],0,670,-255,255);
          y = map(data[1],0,670,-255,255);
           Serial.print("X = ");
          Serial.print(x);
          Serial.print(" Y = ");      
          Serial.print(y);
          ...
          backward = 335- data[1];
          */         
          if ( left >= 10 && left >= right && left >= forward && left >= backward){
                Serial.println("turn left");
                turn_Left(left);
          }else if ( right >= 10 && right >= left && right >= forward && right >= backward){
                Serial.println("turn right");
                turn_Right(right);
          }else if ( forward >= 10 && forward >= left && forward >= right && forward >= backward){
                Serial.println("go forward");
                move_Forward(forward);
          }else if ( backward >= 10 && backward >= forward && backward >= right && backward >= left){
